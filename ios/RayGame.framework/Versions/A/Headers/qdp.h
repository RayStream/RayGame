...
#define COLOR_RANGE_FULL     1
    ...
    typedef struct _STREAM_CONFIGURATION_QDP {
        ...
        // 是否捕获鼠标
        uint8_t captureCursor;
        
    } STREAM_CONFIGURATION_QDP, *PSTREAM_CONFIGURATION_QDP;
    
    // Use this function to zero the stream configuration when allocated on the stack or heap
    void LiInitializeStreamConfigurationQDP(PSTREAM_CONFIGURATION_QDP streamConfig);
...
#define CAPABILITY_SLICES_PER_FRAME(x) (((unsigned char)(x)) << 24)
    ...
    typedef int(*DecoderRendererSetup_QDP)(int videoFormat, int width, int height, int redrawRate, void* context, int drFlags);
    
    // This callback notifies the decoder that the stream is starting. No frames can be submitted before this callback returns.
    typedef void(*DecoderRendererStart_QDP)(void);

    typedef void(*DecoderRendererServerDisplay_QDP)(int width, int height);
    ...
    typedef void(*DecoderRendererStop_QDP)(void);
    
    // This callback performs the teardown of the video decoder. No more frames will be submitted when this callback is invoked.
    typedef void(*DecoderRendererCleanup_QDP)(void);

    // 鼠标状态回调
    typedef void(*CursorData)(char* pixels, int pixels_count, int width, int height, int depth, int pitch, int x_hotspot, int y_hotspot, char show);

    typedef void(*VideoData_QDP)(char *data, int length);
...
//    typedef int(*DecoderRendererSubmitDecodeUnit)(PDECODE_UNIT decodeUnit);
    ...
    typedef int(*DecoderRendererSubmitDecodeUnit_QDP)(char *data, int length, int fullLenth, int bufferType, int frameNumber, long receiveTimeMs);
    
    typedef struct _DECODER_RENDERER_CALLBACKS_QDP {
        DecoderRendererSetup_QDP setup;
        DecoderRendererStart_QDP start;
        DecoderRendererStop_QDP stop;
        DecoderRendererServerDisplay_QDP serverDisplay;
        DecoderRendererCleanup_QDP cleanup;
        DecoderRendererSubmitDecodeUnit_QDP submitDecodeUnit;
        CursorData cursorData;
        VideoData_QDP vidoeData;
        int capabilities;
        
    } DECODER_RENDERER_CALLBACKS_QDP, *PDECODER_RENDERER_CALLBACKS_QDP;
    
    // Use this function to zero the video callbacks when allocated on the stack or heap
    void LiInitializeVideoCallbacksQDP(PDECODER_RENDERER_CALLBACKS_QDP drCallbacks);
    ...
    typedef struct _OPUS_MULTISTREAM_CONFIGURATION_QDP {
        int sampleRate;
        int channelCount;
        int streams;
        int coupledStreams;
    ...
    } OPUS_MULTISTREAM_CONFIGURATION_QDP, *POPUS_MULTISTREAM_CONFIGURATION_QDP;
    ...
    typedef int(*AudioRendererInit_QDP)(int audioConfiguration, const POPUS_MULTISTREAM_CONFIGURATION_QDP opusConfig, void* context, int arFlags);
    
    // This callback notifies the decoder that the stream is starting. No audio can be submitted before this callback returns.
    typedef void(*AudioRendererStart_QDP)(void);
    ...
    typedef void(*AudioRendererStop_QDP)(void);
    ...
    typedef void(*AudioRendererCleanup_QDP)(void);
    
    // This callback provides Opus audio data to be decoded and played. sampleLength is in bytes.
    typedef void(*AudioRendererDecodeAndPlaySample_QDP)(uint8_t *sampleData, uint32_t sampleLength, uint64_t timestamp);
    
    typedef struct _AUDIO_RENDERER_CALLBACKS_QDP {
        AudioRendererInit_QDP init;
        AudioRendererStart_QDP start;
        AudioRendererStop_QDP stop;
        AudioRendererCleanup_QDP cleanup;
        AudioRendererDecodeAndPlaySample_QDP decodeAndPlaySample;
        int capabilities;
    } AUDIO_RENDERER_CALLBACKS_QDP, *PAUDIO_RENDERER_CALLBACKS_QDP;
    
    // Use this function to zero the audio callbacks when allocated on the stack or heap
    void LiInitializeAudioCallbacksQDP(PAUDIO_RENDERER_CALLBACKS_QDP arCallbacks);
...
#define STAGE_MAX 12
    ...
    typedef void(*ConnListenerStageStartingQDP)(int stage);
    
    // This callback is invoked to indicate that a stage of initialization has completed
    typedef void(*ConnListenerStageCompleteQDP)(int stage);
    ...
    typedef void(*ConnListenerStageFailedQDP)(int stage, int errorCode);
    
    // This callback is invoked after the connection is successfully established
    typedef void(*ConnListenerConnectionStartedQDP)(void);
    ...
    typedef void(*ConnListenerConnectionTerminatedQDP)(int errorCode);
...
#define ML_ERROR_NO_VIDEO_TRAFFIC -100
    ...
    typedef void(*ConnListenerLogMessageQDP)(const char* format, ...);
    ...
    typedef void(*ConnListenerRumbleQDP)(unsigned short controllerNumber, unsigned short lowFreqMotor, unsigned short highFreqMotor);
...
#define CONN_STATUS_POOR    1
    typedef void(*ConnListenerConnectionStatusUpdateQDP)(int connectionStatus);
    
    typedef struct _CONNECTION_LISTENER_CALLBACKS_QDP {
        ConnListenerStageStartingQDP stageStarting;
        ConnListenerStageCompleteQDP stageComplete;
        ConnListenerStageFailedQDP stageFailed;
        ConnListenerConnectionStartedQDP connectionStarted;
        ConnListenerConnectionTerminatedQDP connectionTerminated;
        ConnListenerLogMessageQDP logMessage;
        ConnListenerRumbleQDP rumble;
        ConnListenerConnectionStatusUpdateQDP connectionStatusUpdate;
    } CONNECTION_LISTENER_CALLBACKS_QDP, *PCONNECTION_LISTENER_CALLBACKS_QDP;
    
    // Use this function to zero the connection callbacks when allocated on the stack or heap
    void LiInitializeConnectionCallbacksQDP(PCONNECTION_LISTENER_CALLBACKS_QDP clCallbacks);
    
    
    typedef struct _SERVER_INFORMATION_QDP {
        ...
        // 验证方式
        int authenticate_mode;
        
        // 验证信息
        const char* authenticate_data1;
        const char* authenticate_data2;
        ...
        // tcp 网关信息
        const char* gateway_tcp;
        unsigned short gateway_tcp_port;
        
        // udp 网关信息
        const char *gateway_udp;
        unsigned short gateway_udp_port;
        
        // 网络发生过变化
        unsigned char network_switched;
        
        // 过期key
        uint32_t expire_key;
        
    } SERVER_INFORMATION_QDP, *PSERVER_INFORMATION_QDP;
    
    // Use this function to zero the server information when allocated on the stack or heap
    void LiInitializeServerInformationQDP(PSERVER_INFORMATION_QDP serverInfo);
    ...
    //
    int LiStartConnectionQDP(PSERVER_INFORMATION_QDP serverInfo, PSTREAM_CONFIGURATION_QDP streamConfig, PCONNECTION_LISTENER_CALLBACKS_QDP clCallbacks,
                          PDECODER_RENDERER_CALLBACKS_QDP drCallbacks, PAUDIO_RENDERER_CALLBACKS_QDP arCallbacks, void* renderContext, int drFlags,
                          void* audioContext, int arFlags);
    
    // This function stops streaming. This function is not thread-safe.
    void LiStopConnectionQDP(void);
    ...
    void LiInterruptConnectionQDP(void);
    ...
    const char* LiGetStageNameQDP(int stage);
    
    // This function queues a relative mouse move event to be sent to the remote server.
    int LiSendMouseMoveEventQDP(short deltaX, short deltaY);

    // This function queues a relative mouse move to point event to be sent to the remote server.
    int LiSendMouseMoveToPointEventQDP(short deltaX, short deltaY);

    int LiSendMouseMoveToPointADeltaEventQDP(short x, short y, short dx, short dy);
...
#define BUTTON_X2 0x05
    int LiSendMouseButtonEventQDP(char action, int button);
...
#define MODIFIER_META 0x08
    int LiSendKeyboardEventQDP(short keyCode, char keyAction, char modifiers);
...
//    int LiSendControllerEventQDP(short buttonFlags, unsigned char leftTrigger, unsigned char rightTrigger,
...
//    int LiSendMultiControllerEventQDP(short controllerNumber, short activeGamepadMask,
...
//                                   short leftStickX, short leftStickY, short rightStickX, short rightStickY);
    ...
    int LiSendScrollEventQDP(signed char scrollClicks);
    ...
    int LiSendHighResScrollEventQDP(short scrollAmount);
    ...
    // network byte order.
    int LiFindExternalAddressIP4QDP(const char* stunServer, unsigned short stunPort, unsigned int* wanAddr);

    int LiSendStopCaptureQDP(void);

    int LiSendStopDataQDP(void);

    int LiSendVideoParamsQDP(void);

    int LiSendDataStartQDP(void);

    int LiSendCaptureStartQDP(void);

void LiSetSteamConfigQDP(PSTREAM_CONFIGURATION_QDP streamConfig);
...
//    int LiGetPendingVideoFramesQDP(void);
...
//    int LiGetPendingAudioFramesQDP(void);
...
//    int LiGetPendingAudioDurationQDP(void);
...
//    unsigned int LiGetPortFlagsFromStageQDP(int stage);
//    unsigned int LiGetPortFlagsFromTerminationErrorCodeQDP(int errorCode);
    
    // Returns the IPPROTO_* value for the specified port index
//    int LiGetProtocolFromPortFlagIndexQDP(int portFlagIndex);
    
    // Returns the port number for the specified port index
//    unsigned short LiGetPortFromPortFlagIndexQDP(int portFlagIndex);
...
//    unsigned int LiTestClientConnectivityQDP(const char* testServer, unsigned short referencePort, unsigned int testPortFlags);
    
    
    
    
    // host 字节序转 network 字节序
    void host_to_net(void * obj, size_t size);
    
    // network 字节序转 host 字节序
    void net_to_host(void * obj, size_t size);
    
    // 发送鼠标键盘数据
    uint8_t send_mk_data(uint8_t message_type, const void * packet_data, uint32_t packet_data_size, const void * data, uint32_t data_size);
    
    // 发送控制流可写事件
    void send_control_writable_event();
