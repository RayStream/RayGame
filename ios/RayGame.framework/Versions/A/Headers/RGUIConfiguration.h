...
//  rgui
//
//  Created by RGUI Team on 15/3/29.
//
...
@protocol RGUIConfigurationTemplateProtocol <NSObject>
...
@optional
/// 当返回 YES 时，启动 App 的时候 RGUIConfiguration 会自动应用这份配置表。但启动 App 时自动应用的配置表最多只允许一份，如果有多份则其他的会被忽略
/// RGUIConfiguration automatically applies this template on launch when set to YES. Since only one copy of configuration template is allowed when the app launches, you'll have to call `applyConfigurationTemplate` manually if you have more than one configuration templates. 
- (BOOL)shouldApplyTemplateAutomatically;

@end

/**
 *  维护项目全局 UI 配置的单例，通过业务项目自己的 RGUIConfigurationTemplate 来为这个单例赋值，而业务代码里则通过 RGUIConfigurationMacros.h 文件里的宏来使用这些值。
 *  A singleton that contains various UI configurations. Use `RGUIConfigurationTemplate` to set values; Use macros in `RGUIConfigurationMacros.h` to get values.
 */
@interface RGUIConfiguration : NSObject
...
@property(nonatomic, assign) CGFloat            windowLevelRGUIAlertView;
@property(nonatomic, assign) CGFloat            windowLevelRGUIConsole;
...
@property(nonatomic, assign) BOOL               shouldPrintRGUIWarnLogToConsole;
...
@property(nonatomic, assign) BOOL               sendAnalyticsToRGUITeam;
...
@interface UINavigationBar (RGUIConfiguration)
 ...
 */
+ (instancetype)rgui_appearanceConfigured;
@end

@interface UITabBar (RGUIConfiguration)
...
@interface UIToolbar (RGUIConfiguration)
...
@interface UITabBarItem (RGUIConfiguration)
