...
//  RGUIKit
//
//  Created by RGUI Team on 2018/8/14.
//

#import <objc/runtime.h>
#import <UIKit/UIKit.h>
#import <Foundation/Foundation.h>
#import "NSObject+RGUI.h"
#import "NSMethodSignature+RGUI.h"
#import "RGUILog.h"

/// 以高级语言的方式描述一个 objc_property_t 的各种属性，请使用 `+descriptorWithProperty` 生成对象后直接读取对象的各种值。
@interface RGUIPropertyDescriptor : NSObject
...
OverrideImplementation(Class targetClass, SEL targetSelector, id (^implementationBlock)(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void))) {
    ...
    IMP (^originalIMPProvider)(void) = ^IMP(void) {
        ...
        if (hasOverride) {
            ...
            // https://github.com/Tencent/RGUI_iOS/issues/776
    ...
    if (hasOverride) {
        ...
        const char *typeEncoding = method_getTypeEncoding(originMethod) ?: [targetClass instanceMethodSignatureForSelector:targetSelector].rgui_typeEncoding;
 ...
 */
#define _RGUITypeEncodingDetectorGenerator(_TypeInFunctionName, _typeForEncode) \
...
_RGUITypeEncodingDetectorGenerator(Char, char)
_RGUITypeEncodingDetectorGenerator(Int, int)
_RGUITypeEncodingDetectorGenerator(Short, short)
_RGUITypeEncodingDetectorGenerator(Long, long)
_RGUITypeEncodingDetectorGenerator(LongLong, long long)
_RGUITypeEncodingDetectorGenerator(NSInteger, NSInteger)
_RGUITypeEncodingDetectorGenerator(UnsignedChar, unsigned char)
_RGUITypeEncodingDetectorGenerator(UnsignedInt, unsigned int)
_RGUITypeEncodingDetectorGenerator(UnsignedShort, unsigned short)
_RGUITypeEncodingDetectorGenerator(UnsignedLong, unsigned long)
_RGUITypeEncodingDetectorGenerator(UnsignedLongLong, unsigned long long)
_RGUITypeEncodingDetectorGenerator(NSUInteger, NSUInteger)
_RGUITypeEncodingDetectorGenerator(Float, float)
_RGUITypeEncodingDetectorGenerator(Double, double)
_RGUITypeEncodingDetectorGenerator(CGFloat, CGFloat)
_RGUITypeEncodingDetectorGenerator(BOOL, BOOL)
_RGUITypeEncodingDetectorGenerator(Void, void)
_RGUITypeEncodingDetectorGenerator(Character, char *)
_RGUITypeEncodingDetectorGenerator(Object, id)
_RGUITypeEncodingDetectorGenerator(Class, Class)
_RGUITypeEncodingDetectorGenerator(Selector, SEL)
...
#define _RGUIGetIvarValueGenerator(_TypeInFunctionName, _typeForEncode) \
...
_RGUIGetIvarValueGenerator(Char, char)
_RGUIGetIvarValueGenerator(Int, int)
_RGUIGetIvarValueGenerator(Short, short)
_RGUIGetIvarValueGenerator(Long, long)
_RGUIGetIvarValueGenerator(LongLong, long long)
_RGUIGetIvarValueGenerator(UnsignedChar, unsigned char)
_RGUIGetIvarValueGenerator(UnsignedInt, unsigned int)
_RGUIGetIvarValueGenerator(UnsignedShort, unsigned short)
_RGUIGetIvarValueGenerator(UnsignedLong, unsigned long)
_RGUIGetIvarValueGenerator(UnsignedLongLong, unsigned long long)
_RGUIGetIvarValueGenerator(Float, float)
_RGUIGetIvarValueGenerator(Double, double)
_RGUIGetIvarValueGenerator(BOOL, BOOL)
_RGUIGetIvarValueGenerator(Character, char *)
_RGUIGetIvarValueGenerator(Selector, SEL)
...
/**
 ...
 int count = rgui_getProjectClassList(&classes);
 Class class = (__bridge Class)classes[0];
 @endcode
 */
FOUNDATION_EXPORT int rgui_getProjectClassList(classref_t **classes);
/**
 检测是否存在某个dyld  image
 */
FOUNDATION_EXPORT BOOL rgui_exists_dyld_image(const char *target_image_name);
