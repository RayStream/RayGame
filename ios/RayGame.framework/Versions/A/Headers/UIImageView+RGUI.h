...
/**
 是否要用 RGUI 提供的高性能方式去渲染由 [UIImage animatedImage] 创建的 UIImage，（系统原生的方式在 UIImageView 被放在 UIScrollView 内时会卡顿），默认为 NO。
 */
@property(nonatomic, assign) BOOL rgui_smoothAnimation;
 ...
 *  建议在设置完x/y之后调用
 */
- (void)rgui_sizeToFitKeepingImageAspectRatioInSize:(CGSize)limitSize;
@end
