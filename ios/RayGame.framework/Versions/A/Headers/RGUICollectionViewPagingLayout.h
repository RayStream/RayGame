...
//
//  RGUICollectionViewPagingLayout.h
//  rgui
//
//  Created by RGUI Team on 15/9/24.
//

#import <UIKit/UIKit.h>

typedef NS_ENUM(NSInteger, RGUICollectionViewPagingLayoutStyle) {
    RGUICollectionViewPagingLayoutStyleDefault, // 普通模式，水平滑动
    RGUICollectionViewPagingLayoutStyleScale,   // 缩放模式，两边的item会小一点，逐渐向中间放大
    RGUICollectionViewPagingLayoutStyleRotation // 旋转模式，围绕底部某个点为中心旋转
};
 ...
 */
@interface RGUICollectionViewPagingLayout : UICollectionViewFlowLayout

- (instancetype)initWithStyle:(RGUICollectionViewPagingLayoutStyle)style NS_DESIGNATED_INITIALIZER;

@property(nonatomic, assign, readonly) RGUICollectionViewPagingLayoutStyle style;
...
@interface RGUICollectionViewPagingLayout (DefaultStyle)
...
@interface RGUICollectionViewPagingLayout (ScaleStyle)
...
extern const CGFloat RGUICollectionViewPagingLayoutRotationRadiusAutomatic;

@interface RGUICollectionViewPagingLayout (RotationStyle)

/**
 *  旋转卡片相关
 *  左右两个卡片最终旋转的角度有 rotationRadius * 90 计算出来
 *  rotationRadius表示旋转的半径
 *  @warning 仅当 style 为 RGUICollectionViewPagingLayoutStyleRotation 时才生效
 */
