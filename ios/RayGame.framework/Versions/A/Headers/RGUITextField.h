...
//  rgui
//
//  Created by RGUI Team on 16-11-03
//
...
@protocol RGUITextFieldDelegate <UITextFieldDelegate>
 ...
 */
- (void)textField:(RGUITextField *)textField didPreventTextChangeInRange:(NSRange)range replacementString:(NSString *)replacementString;
 ...
 */
@interface RGUITextField : UITextField

@property(nonatomic, weak) id<RGUITextFieldDelegate> delegate;
...
/**
 *  在使用 maximumTextLength 功能的时候，是否应该把文字长度按照 [NSString (RGUI) rgui_lengthWhenCountingNonASCIICharacterAsTwo] 的方法来计算。
 *  默认为 NO。
 */
